...
tabs_router = APIRouter(prefix="/tabs", dependencies=[Depends(get_current_user)])

@tabs_router.get("", response_model=list[Tab])
...
@tabs_router.post("", response_model=Tab)
async def create_tab(tab: TabCreate) -> dict:
    print("hi")
    new_tab = await add_tab(tab.model_dump())
    return new_tab

@tabs_router.patch("/{tab_id}", response_model=Tab)
...
@tabs_router.delete("/{tab_id}", response_model=dict)
